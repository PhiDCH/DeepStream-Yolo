...
class STrack(BaseTrack):
    ...
    def __init__(self, tlwh, score, cl):
    ...
    def tlbr_id(self):
        ...
        """
        tlbr = self.tlbr.copy()
        return np.concatenate([tlbr, [self.track_id], [self.cl]])
...
class BYTETracker(object):
    def __init__(self, args, max_age=300):
    ...
    def update(self, output_results, img_info, img_size):
        ...
        for inew in u_detection:
            ...
            if track.score < self.det_thresh:
                continue
            self.track_id += 1
            track.activate(self.kalman_filter, self.frame_id, self.track_id)
            activated_starcks.append(track)
        """ Step 5: Update state"""
        ...
        output_stracks = [track.tlbr_id for track in self.tracked_stracks if track.is_activated]
        if len(output_stracks):
            output = np.concatenate(output_stracks).reshape(-1, 6)
            return output
        else:
            return []
